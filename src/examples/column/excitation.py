"""Dynamic base excitation + intensity tuning for the column seismic studies (shared by dynamic.py
and dynamic_linear.py).
...
chosen record to a target peak roof drift on the cheap fiber column. Both records are returned in g,
so the same `scale = G * intensity` machinery in the runners applies to either. Units: kip, in.
"""
